...
par_path <- path_tab %>%
    unnest(path_theta) %>%
    mutate(repar_xi = map_dbl(path_av_theta, ~mean((.x[1]-repar_theta[1])^2)),
           repar_rho = map_dbl(path_av_theta, ~mean((.x[2]-repar_theta[2])^2)),
           intercepts = map_dbl(path_av_theta, ~mean((.x[(3+m):d]-repar_theta[(3+m):d])^2)),
           all = map_dbl(path_av_theta, ~mean((.x-repar_theta)^2))) %>%
    filter(!(stoc_seed %in% c(54, 422))) %>%
    gather(key = 'par_type', value = 'mse',repar_xi, repar_rho, intercepts,  all) %>%
    group_by(sample_size, meth, stepsize, scaling, iter, par_type) %>%
    summarise(mse = mean(mse), q25 = quantile(mse, .25), q50 = quantile(mse, .50), q75 = quantile(mse, .75))
...
mse <- par_path %>%
    ...
    geom_hline(data = num_par %>%
               ...
               aes(yintercept = log(mse)), col = numcol, linetype = 'dashed', alpha = 0.5)+
    geom_line(aes(col = factor(meth)), alpha = .6, linewidth = 1) +
...
mse_tot <- par_path %>%
    filter(stepsize == 2, par_type == 'all') %>%
    mutate(pass = iter/sample_size) %>%
    ggplot(aes(x = pass, y = log(mse)))+
    geom_rect(aes(xmin=-Inf,xmax=burn,ymin=-Inf,ymax=Inf),
              fill="lightgrey", color =NA)+
    geom_hline(data = num_par %>%  filter(par_type == 'all'),
               aes(yintercept = log(mse)), col = numcol, linetype = 'dashed', alpha = 0.5)+
    geom_line(aes(col = factor(meth)), alpha = .6, linewidth = 1) +
...
par_path %>%
    ...
    geom_rect(aes(xmin=-Inf,xmax=burn,ymin=-Inf,ymax=Inf),
              fill="lightgrey", color =NA)+
    geom_line(aes(col = factor(meth)), alpha = .6, linewidth = 1) +
    geom_point(data = num_par %>%  filter(par_type == 'all') %>% ungroup() %>% select(-stepsize) %>% expand_grid(stepsize = unique(sim_settings$stepsize)),
               aes(x = npasses, y = log(mse)), col = numcol, shape = 4, size = 2, alpha = .6)+
    facet_nested(~sample_size+stepsize, scales = 'free') +
    theme_bw()+
    labs(col = 'Sampling scheme:', x = 'Number of passes through the data', y = 'Average mse') +
    scale_color_manual(values = colz) +
    theme(legend.position = 'bottom')
...
var_tr <- sim_trace %>%
    filter(stepsize == 2, sample_size != 100000) %>%
    ggplot(aes(x = pass, y = (tr)))+
    geom_hline(data = num_sim_variance %>%
                   expand_grid(stepsize=unique(stoc_est$stepsize)),
               aes(yintercept = (tr)), col = numcol, linetype = 'dashed', alpha = .5) +
...
num_coverage <- num_est %>%
    mutate(
        coverage_tot  = map2(mod, asy_var, ~ repar_theta > .x$theta-1.96*.y$se$se_tot  & repar_theta < .x$theta+1.96*.y$se$se_tot )
    ) %>%
    mutate(
...
st_coverage <- path_selected %>%
    mutate(
        coverage_tot  = map2(path_av_theta, asy_var, ~ repar_theta > .x-1.96*.y$se$se_tot  & repar_theta < .x+1.96*.y$se$se_tot ),
        coverage_stat = map2(path_av_theta, asy_var, ~ repar_theta > .x-1.96*.y$se$se_stat & repar_theta < .x+1.96*.y$se$se_stat),
        coverage_stoc = map2(path_av_theta, asy_var, ~ repar_theta > .x-1.96*.y$se$se_stoc & repar_theta < .x+1.96*.y$se$se_stoc)
    ) %>%
    select(-asy_var) %>%
    mutate(
        coverage_tot = lapply(coverage_tot, function(x){
